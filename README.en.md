...
> You'll receive a **`Blog.zip`** release package (the maintainer generates it
> with `scripts/package-release.sh`). Unzip it to get a `Blog` folder and put
> the **whole folder** on your Desktop or in Documents.
...
```bash
./scripts/package-release.sh   # produces dist-release/Blog/ and a sendable Blog.zip
```

The output has cross-platform self-contained binaries + per-OS double-click
launchers (`Start-Blog.app` / `.vbs` / `.command` / `.bat`) + the startup
`loading.html` + `docs/新手指南.md`.

> **Client directory**: the "client" you hand to end users is the packaged
> **`dist-release/Blog/`** folder (zipped as `Blog.zip`) — double-click launchers
> plus a self-contained binary, no toolchain required. It is generated by the
> script and **not tracked in git** (see `.gitignore`). The launcher **sources**
> live in the repo's **`packaging/`** directory (`mac/` `.app` shell + `launch`,
> `windows/` `.vbs`, and the shared `loading.html`), together with the
> root-level `Start-Blog.command` / `Start-Blog.bat`.
...
packaging/                  client launcher sources (bundled into the dist-release/Blog/ client)
